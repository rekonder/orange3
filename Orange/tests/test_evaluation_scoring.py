...
from Orange.evaluation import AUC, CA, Results, Recall, \
    Precision, TestOnTrainingData, scoring, LogLoss, F1, CrossValidation, testing
...
class Scoring_AUC_Test(unittest.TestCase):
    ...
    def test_multiclass_auc_multi_learners(self):
        learners = [LogisticRegressionLearner(),
                    MajorityLearner()]
        res = testing.CrossValidation(self.iris, learners, k=10)
    ...
    def test_auc_scores(self):
        ...
        for predict, prob in (([1., 1., 1., 0., 0., 0.], 0.),      # All wrong
            ...
            self.assertAlmostEqual(self.compute_auc(actual, predict), prob)
